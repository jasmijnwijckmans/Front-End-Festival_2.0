        ...
        .then(function (returndata) {
            ...
            if (returndata.success) {
                ...
                returndata.data.forEach(function (stage) {
                    ...
                    row += " <div class='card mt-4 w-100 h-80'>"
                    ...
                    row += "<div class= 'card-title'>" + stage.stageName + "</div><div><div class='divider div-transparent'></div><u>Current Song</u>:<br>" + stage.currentSong + "<div class='divider div-transparent'></div><u>Genre</u>: " + stage.stageGenre +"<br><u>Number of Users</u>: " + stage.numberOfUsers + "<br> <a><button class='btn' value='Join'  onclick='UpdateActivity(" + stage.stageID + ")'> Join<i class=''></i></button></a></div>"
                    row += "</div>"
                    row += "</div>"
                    //console.log(row);
                });
                if (localStorage.getItem("UserRole") == "admin") { // when userID is admin, show this table.
                    ...
                    row += "<div class='card-title'> Manage Stages </div><div><br><button class='btn ' onclick='GoToCreateStage()'> Go </button></div>"
                    row += "</div>"
                    row += "</div>"

                    row += " <div class='col-sm-4 col-lg-4'>"
                    row += " <div class='card card mt-4 w-100 h-80'>"
                    ...
                    row += "<div class='card-title'>  Manage users <br></div><div><button class='btn' onclick='GoToManageUsers()'> Go</button></div>"
                    row += "</div>"
                    row += "</div>"
                }
